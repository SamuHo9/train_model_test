...
def run_pipeline():
    ...
    plt.ylabel('Loss')
    plt.grid(True)
    plt.savefig('plots/mobilenet_loss_curve_full.png')
    plt.close()
    ...
    plt.savefig('plots/mobilenet_confusion_matrix_full.png')
    plt.close()
    ...
    plt.title('Receiver Operating Characteristic (ROC) - Test Set (MobileNetV2)')
    plt.legend(loc="lower right")
    plt.grid(True)
    plt.savefig('plots/mobilenet_roc_curve_full.png')
    plt.close()
