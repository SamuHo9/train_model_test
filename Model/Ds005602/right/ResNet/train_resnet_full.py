...
def run_pipeline():
    ...
    for train_idx, val_idx in cv.split(X_train, y_train):
        ...
        _, _, val_acc = train_resnet_model(
            ...
            epochs=50, batch_size=256, device=device
        )
    ...
    final_model, losses, _ = train_resnet_model(
        X_train_pls, y_train, epochs=100, batch_size=256, device=device
    )
    ...
    plt.ylabel('Loss')
    plt.grid(True)
    plt.savefig('plots/resnet_loss_curve_full.png')
    plt.close()
    ...
    plt.title('Confusion Matrix - Test Set (ResNet)')
    ...
    plt.title('Receiver Operating Characteristic (ROC) - Test Set (ResNet)')
