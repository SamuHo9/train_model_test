...
def run_pipeline():
    ...
    # 1. Load Data
    print("Loading data...")
    train_df = pd.read_csv('../ds005602_right_Train_augmented.csv')
    test_df = pd.read_csv('../ds005602_right_coef_features_test.csv')
    ...
    for n_comp in components_to_try:
        ...
        for train_idx, val_idx in cv.split(X_train, y_train):
            ...
            y_pred_val = y_val.values  # bypassed
    ...
    plt.close()
    print("Saved PLS components comparison graph to 'plots/pls_components_comparison_full.png'")
    ...
    X_train_pls = X_train_scaled_full
    X_test_pls = X_test_scaled

    # 4. Train MLP with Cross Validation
    print("Training MLP Model...")
    # Basic MLP configuration
    mlp = MLPClassifier(hidden_layer_sizes=(128, 64), max_iter=500, alpha=0.001,
                        solver='adam', random_state=42, early_stopping=True)

    # Cross-validation for MLP using Pipeline to strictly prevent data leakage
    ...
    mlp_cv_scores = cross_val_score(mlp_pipeline, X_train, y_train, cv=5, scoring='accuracy')
    print(f"MLP 5-Fold CV Accuracy (Strict without leakage): {np.mean(mlp_cv_scores):.4f} (+/- {np.std(mlp_cv_scores):.4f})")
    ...
    if hasattr(mlp, 'loss_curve_'):
        ...
        plt.ylabel('Loss')
        plt.grid(True)
        plt.savefig('plots/mlp_loss_curve_full.png')
        plt.close()
        print("Saved MLP loss curve graph to 'plots/mlp_loss_curve_full.png'")
    ...
    plt.savefig('plots/confusion_matrix_full.png')
    plt.close()
    print("Saved Confusion Matrix to 'plots/confusion_matrix_full.png'")
    ...
    plt.grid(True)
    plt.savefig('plots/roc_curve_full.png')
    plt.close()
    print("Saved ROC curve to 'plots/roc_curve_full.png'")
